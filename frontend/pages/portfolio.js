...
export default function Portfolio() {
    ...
    async function cancelOrder(orderId, orderType, asset, quantity, price) {
        if (!confirm(`Êtes-vous sûr de vouloir annuler cet ordre ${orderType} ?`)) return;
        ...
        try {
            ...
            if (result.success) {
                alert(`✅ Ordre annulé !\n${result.refundDetails.amount} remboursés`);
                await loadAllData(account); // Recharger les données
            } else {
    ...
    const walletValue = Object.entries(balances).reduce((total, [symbol, balance]) => {
        return total + (parseFloat(balance) * prices[symbol]);
    }, 0);
    const vaultValue = Object.entries(vaultBalances).reduce((total, [symbol, balance]) => {
        return total + (parseFloat(balance) * prices[symbol]);
    }, 0);
    ...
    if (!account) {
        return (
            <div style={{ padding: '20px' }}>
                <h2>⚠️ Wallet non connecté</h2>
                <p>Veuillez connecter votre wallet pour voir votre portfolio.</p>
                <Link href="/" style={{ color: '#007bff' }}>← Retour à l'accueil</Link>
            </div>
        );
    }
                ...
                <>
                    ...
                    <div style={{ 
                        ...
                        <div style={{ 
                            ...
                            <h3 style={{ margin: '0 0 10px 0', color: '#155724' }}>💰 Wallet</h3>
                        ...
                        <div style={{ 
                            ...
                            <h3 style={{ margin: '0 0 10px 0', color: '#856404' }}>🏦 Vault (Escrow)</h3>
                            <p style={{ margin: 0, fontSize: '18px', fontWeight: 'bold' }}>
                        ...
                        <div style={{ 
                            ...
                            <h3 style={{ margin: '0 0 10px 0', color: '#0c5460' }}>💎 Total</h3>
                            <p style={{ margin: 0, fontSize: '18px', fontWeight: 'bold' }}>
                        ...
                        <table style={{ 
                            ...
                            <thead>
                                <tr style={{ backgroundColor: '#007bff', color: 'white' }}>
                                    ...
                                    <th style={{ padding: '12px', border: '1px solid #0056b3' }}>💰 Wallet</th>
                                    <th style={{ padding: '12px', border: '1px solid #0056b3' }}>🏦 Vault (Escrow)</th>
                                    <th style={{ padding: '12px', border: '1px solid #0056b3' }}>💎 Total</th>
                                        ...
                                        <tr key={symbol} style={{ 
                                            ...
                                            <td style={{ padding: '12px', border: '1px solid #dee2e6', fontWeight: 'bold' }}>
                                                <Link href={`/assets/${symbol}`} style={{ 
                                                    ...
                                                    {symbol} {symbol === 'TRG' && '🟡'} {symbol === 'CLV' && '🏢'} {symbol === 'ROO' && '🌿'}
                                                </Link>
                                            </td>
                                            <td style={{ 
                                                padding: '12px', 
                                            ...
                                            <td style={{ 
                                                ...
                                                {vaultBal > 0 && <span style={{ color: '#856404' }}> (bloqué)</span>}
                                            </td>
                                            <td style={{ 
                                                padding: '12px', 
                                                border: '1px solid #dee2e6',
                    ...
                    {/* Mes ordres avec possibilité d'annulation */}
                    <div style={{ marginBottom: '30px' }}>
                        <h3>📋 Mes ordres (avec annulation)</h3>
                        {myOrders.length === 0 ? (
                            <p style={{ 
                    ...
                    <div style={{ 
                        ...
                        <h4>🎯 Explications pour l'audit</h4>
